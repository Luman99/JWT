...
const EditExamPage = () => {
  ...
  useEffect(() => {
    const fetchExam = async () => {
      try {
        const response = await axios.get(`http://ec2-18-159-196-177.eu-central-1.compute.amazonaws.com:8000/api/exams/${id}/`, {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.patch(
        `http://ec2-18-159-196-177.eu-central-1.compute.amazonaws.com:8000/api/update_exam/${id}/`,
        {
          name,
          description,
